...
O usuário digita um número no campo de texto e clica em **Gerar**. O JavaScript calcula a tabuada daquele número (de 1 a 10) e exibe os resultados em um `<select>`, substituindo o conteúdo de exemplo que já estava no HTML.
...
- A lógica principal está em um `addEventListener` no botão **Gerar**
- Se o campo estiver vazio, o `<select>` exibe a mensagem "Campo Vazio!" e o `size` é reduzido para 1
- Se o campo tiver um valor, o `size` volta para 11, e a tabuada é gerada com um loop de 1 a 10
